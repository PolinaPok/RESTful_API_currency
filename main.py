...
app = FastAPI(lifespan=lifespan, redoc_url="/redoc_docs")


# Базовая модель валют
@app.post("/currencies")
async def load_currencies(date: str):
    ...
    try:
        ...
        date_in_past = bool(date_object < datetime.now())
    except ValueError:
    ...
    if not check_saved:
        # response = requests.get(f'https://www.cbr.ru/scripts/XML_daily.asp', params={'date_req': date.replace('-', '/')})
